...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        """[WF-02] Warga tidak boleh mengedit laporan berstatus REPORTED."""
        # ARRANGE
        self.client.force_authenticate(user=self.warga)
        url = f'/api/report/{self.laporan_reported.pk}/'
        payload = {
            'title': 'Judul Coba Diubah Warga',
        ...
        self.assertEqual(
            ...
            "Laporan berstatus REPORTED tidak boleh dimodifikasi oleh warga"
        )
        self.laporan_reported.refresh_from_db()
        self.assertNotEqual(self.laporan_reported.title, 'Judul Coba Diubah Warga')

    # ─────────────────────────────────────────────────────────────────────────
    # WF-05: Laporan RESOLVED Bersifat Read-Only
    # ─────────────────────────────────────────────────────────────────────────
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        """[WF-05] Laporan RESOLVED bersifat read-only, bahkan bagi pemiliknya."""
        # ARRANGE
        self.client.force_authenticate(user=self.warga)
        url = f'/api/report/{self.laporan_resolved.pk}/'
        payload = {
            'title': 'Judul Coba Diubah Setelah Resolved',
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan RESOLVED harus terkunci dari modifikasi apapun"
        )
        self.laporan_resolved.refresh_from_db()
        self.assertNotEqual(self.laporan_resolved.title, 'Judul Coba Diubah Setelah Resolved')
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        self.assertEqual(
            ...
            'VERIFIED',
            "Status di database harus berubah menjadi VERIFIED"
        )

    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        """[WF-04] Transisi REPORTED -> RESOLVED tidak boleh diizinkan."""
        # ARRANGE
        from main_app.views import ALLOWED_TRANSITIONS
        self.client.login(username='admin_portal', password='AdminPass123!')
        url = reverse('update_status', kwargs={'pk': self.laporan_reported.pk})

        # ASSERT bagian 1: cek daftar transisi lokal
        self.assertNotIn(
            'RESOLVED',
            ALLOWED_TRANSITIONS['REPORTED'],
            "RESOLVED tidak boleh ada di daftar transisi valid dari REPORTED"
        )
        self.assertEqual(ALLOWED_TRANSITIONS['REPORTED'], ['VERIFIED'])

        # ACT: coba paksa lompat status via POST
        response = self.client.post(url, {'status': 'RESOLVED'})

        # ASSERT bagian 2: status di database tidak berubah
        self.laporan_reported.refresh_from_db()
        self.assertNotEqual(
            self.laporan_reported.status,
            'RESOLVED',
            "Status tidak boleh lompat langsung dari REPORTED ke RESOLVED"
        )