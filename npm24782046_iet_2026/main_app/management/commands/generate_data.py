...
# Set Faker ke Bahasa Indonesia agar alamat & nama lebih lokal [cite: 26]
fake = Faker('id_ID')

class Command(BaseCommand):
    help = 'Generate contextual fake reports for Lab 7'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Mapping Kategori dengan judul dan deskripsi yang relevan [cite: 34]
        context_data = {
            'Jalan Rusak': {
                'titles': ['Lubang Besar di Tengah Jalan', 'Aspal Mengelupas Parah', 'Jalan Bergelombang Bahayakan Motor', 'Ambles di Dekat Drainase'],
            ...
            'Sampah': {
                'titles': ['Tumpukan Sampah Liar', 'Bau Menyengat Sampah Menumpuk', 'TPS Melebihi Kapasitas', 'Sampah Menutup Saluran Air'],
            ...
            'Lampu Mati': {
                'titles': ['Penerangan Jalan Umum Mati', 'Lampu Jalan Berkedip', 'Kabel Lampu Putus', 'Area Gelap Rawan Kriminalitas'],
            ...
            'Drainase': {
                'titles': ['Saluran Air Mampet', 'Drainase Meluap Saat Hujan', 'Tutup Got Pecah', 'Penyumbatan Karena Sedimen'],
            ...
            'Keamanan': {
                'titles': ['Aksi Vandalisme Fasilitas Umum', 'Pencurian Kabel Telepon', 'Laporan Kerumunan Mencurigakan', 'Gangguan Ketertiban Umum'],
                'desc': 'Dibutuhkan patroli tambahan di area ini karena laporan warga terkait aktivitas yang mencurigakan pada jam malam.'
            }
        }
        
        # PERBAIKAN: Gunakan underscore agar sesuai dengan logika template [cite: 49]
        status_choices = ['REPORTED', 'VERIFIED', 'IN_PROGRESS', 'RESOLVED']

        for _ in range(num_records):
            # Pilih kategori secara acak [cite: 53]
            category = random.choice(list(context_data.keys()))
            
            # Ambil template data berdasarkan kategori terpilih [cite: 54]
            title_template = random.choice(context_data[category]['titles'])
            description_base = context_data[category]['desc']

            Report.objects.create(
                # Judul ditambah sedikit random agar unik [cite: 60]
                title=f"{title_template} - {fake.street_name()}",
                category=category,
                # Deskripsi ditambah detail lokasi dari faker [cite: 63]
