...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        """[AUTH-03] Warga biasa (non-staff) tidak bisa akses /dashboard/."""
        # ARRANGE: login sebagai warga biasa (session-based, bukan JWT)
        ...
        # ASSERT
        # UserPassesTestMixin: user yang SUDAH login tapi gagal test_func()
        # akan menerima PermissionDenied (403), bukan redirect ke login.
        self.assertEqual(
            response.status_code,
            403,
            "Warga non-staff yang sudah login harus ditolak dengan HTTP 403"
        )